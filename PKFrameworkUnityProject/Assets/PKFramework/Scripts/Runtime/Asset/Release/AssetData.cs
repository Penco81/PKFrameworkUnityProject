    ...
    public class AssetData
    {
        ...
        public UnityEngine.Object this[string key]
        {
            get
            {
                ...
                if (_subAssetObjects != null)
                {
                    if (!_subAssetObjects.TryGetValue(key, out assetObject))
                    {
                        if (_assetObject != null && _assetObject is SpriteAtlas atlas)
                        {
                            ...
                            {
                                PKLogger.LogError($"Can not get sub asset({key}) from {_fullPath}");
                            }
                        }
                    }
                }
                ...
                {
                    PKLogger.LogError($"Not Exist sub asset({key}) from {_fullPath}");
                }
        ...
        public void AddRef()
        {
            ++_refCount;
#if UNITY_EDITOR
            PKLogger.LogMessage($"Add AssetData {_fullPath} _refCount = {_refCount}");
#endif
        }
        ...
        public void DecRef(bool bNoDelay = false)
        {
            --_refCount;
#if UNITY_EDITOR
            PKLogger.LogMessage($"Dec AssetData {_fullPath} _refCount = {_refCount}");
#endif
            if (_refCount <= 0)
            {
                if (_refBundle != null)
        ...
        public void LoadAsset(bool withSubAssets = false)
        {
            ...
            if (_refBundle.Bundle != null)
            {
                if (_useSubAsset)
                {
                    ...
                    {
                        PKLogger.LogError($"Can not load the asset '{_fullPath}'");
                    }
                }
                else
                {
        ...
        void OnAssetBundleDataLoadComplete(AssetBundleData bundleData)
        {
            ...
            {
                PKLogger.LogError("Return a mismatch AssetBundleData for OnAssetBundleDataLoadComplete");
            }
        }
        ...
        public void Unload(bool bForce = false)
        {
            ...
            {
                PKLogger.LogWarning($"Try to unload refcount > 0 asset({_fullPath})!");
            }
        }
        #endregion
    }
