...
namespace PKFramework.Runtime.Asset
{
    ...
    public class AssetBundleData
    {
        ...
        /// 初始化该ab的asset信息结构
        /// </summary>
        /// <param name="assets">asset的路径</param>
        ...
        public AssetData GetAsset(string assetPath)
        {
            if (_assets != null)
            {
                ...
                {
                    PKLogger.LogError($"Try to get asset data '{assetPath}' which not in the bundle '{_name}'");
                }
            }
            else
            {
                PKLogger.LogError($"Try to get asset data in the bundle '{_name}' which not include any asset");
            }
        ...
        public void SetAsset(string assetPath, AssetData assetData)
        {
            if (_assets != null)
            {
                ...
                {
                    PKLogger.LogError($"Try to set asset data '{assetPath}' which not in the bundle '{_name}'");
                }
            }
            else
            {
                PKLogger.LogError($"Try to set asset data in the bundle '{_name}' which not include any asset");
            }
        }
        ...
        public void Load(bool bDepended = false)
        {
            if (Bundle == null)
            {
                ...
                for (int i = 0; i < Dependencies.Length; ++i)
                {
                    ...
                    {
                        PKLogger.LogWarning($"Dependency cycle detected for '{Dependencies[i]._name}'");
                    }
                }
                Bundle = AssetBundle.LoadFromFile(FileUtility.GetAssetBundlePathInVersion(_name), 0, Offset);
            ...
            {
                PKLogger.LogError($"Can not load AssetBundle '{_name}'!");
            }
        }
        ...
        public void LoadAsync(System.Action<AssetBundleData> onComplete, bool bDepended = false)
        {
            ...
            if (Bundle == null)
            {
                if (_asyncLoadRequest == null)
                {
                    ...
                    {
                        Request = AssetBundle.LoadFromFileAsync(FileUtility.GetAssetBundlePathInVersion(_name), 0, Offset),
        ...
        public void Unload(bool bForce = false)
        {
            ...
            if (Bundle != null)
            {
                Bundle.Unload(true);
                Bundle = null;
                PKLogger.LogMessage($"Unload Bundle {_name}");
            }
        ...
        public void DecDepRef()
        {
            ...
            {
                _depCount = 0;
                PKLogger.LogWarning($"{_name} _depCount < 0");
            }
        }
        ...
        {
            ++_refCount;
            //PKLogger.LogMessage($"Add AssetBundle {_name} refCount = {_refCount}");
        }
        ...
        {
            --_refCount;
            //PKLogger.LogMessage($"Dec AssetBundle {_name} refCount = {_refCount}");
            if (_refCount == 0)
            {
            ...
            {
                _refCount = 0;
                PKLogger.LogWarning($"{_name} _refCount < 0");
            }
        }
            ...
            if (asyncOperation.isDone)
            {
                if (_asyncLoadRequest != null && _asyncLoadRequest.Request == asyncOperation)
                {
                    ...
                    if (assetBundleCreateRequest != null)
                    {
                        ...
                        {
                            PKLogger.LogError($"Can not load AssetBundle '{_name}' asynchronously!");
                        }
                    }
                ...
                {
                    PKLogger.LogError("Return a mismatch asyncOperation for AssetBundleCreateRequest");
                }
            }
            else
            {
                PKLogger.LogError("Return a not done AsyncOperation for AssetBundleCreateRequest");
            }
        }
        
        //每次依赖加载完调用，可能在加载本ab前可能在后
